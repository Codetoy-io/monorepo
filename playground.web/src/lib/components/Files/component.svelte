<script lang="ts">
    ...
    async function loadAllEntries() {
        try {
            const { listEntriesDetailed } = await import("$lib/files");
    ...
    async function moveFileOperation(sourcePath: string, destinationPath: string) {
        ...
        try {
            const { moveFile } = await import("$lib/files");
    ...
    async function moveFolderOperation(sourcePath: string, destinationPath: string) {
        ...
        try {
            const { moveFolder } = await import("$lib/files");
    ...
    async function deleteEntryOperation(entry: Entry) {
        ...
        try {
            if (entry.kind === "directory") {
                const { deleteFolder } = await import("$lib/files");
...
<div {...props} class="p-2 space-y-1 relative" role="region" aria-label="File Explorer">
    <!-- Context Menu -->
